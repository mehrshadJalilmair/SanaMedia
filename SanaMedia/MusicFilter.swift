...
class MusicFilter: UIViewController , UITableViewDelegate , UITableViewDataSource , UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
    ...
    func GenresSelected()
    {
        ...
        let splited = MusicGenres[selectedGenreIndex].split(separator: "-")
        let genre_first_level = String(splited[0])
        
        let url_dynamic_part:String = String.localizedStringWithFormat(singleton.URLS["musics_by_genre"]!, "\(pageSize)" , "\(newsPageIndex+1)" , genre_first_level.addingPercentEncoding(withAllowedCharacters:NSCharacterSet.urlQueryAllowed)!)
        let url = Singleton.getInstance().url_static_part + url_dynamic_part
