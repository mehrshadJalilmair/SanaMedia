...
var SerialGenres:[String] = [String]()
var FilterSerials:[MovieSerial] = [MovieSerial]()

class SerialFilter: UIViewController , UITableViewDelegate , UITableViewDataSource , UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
    ...
    @objc func GenresClicked()
    {
        ...
        let url_dynamic_part:String = String.localizedStringWithFormat(singleton.URLS["genres"]!, "movie_serial")
    ...
    func GenresSelected()
    {
        //let genre_first_level = SerialGenres[selectedGenreIndex]
        let splited = SerialGenres[selectedGenreIndex].split(separator: "-")
        let genre_first_level = String(splited[0])
        
        let url_dynamic_part:String = String.localizedStringWithFormat(singleton.URLS["movies_serial_by_genre"]!, "\(pageSize)" , "\(newsPageIndex+1)" , genre_first_level.addingPercentEncoding(withAllowedCharacters:NSCharacterSet.urlQueryAllowed)!)
        let url = Singleton.getInstance().url_static_part + url_dynamic_part
        ...
        Alamofire.request(url , method: .get , encoding: URLEncoding.default).validate().responseJSON { response in
            ...
            case .success:
                ...
                let images = data["movie_serial"] as! [AnyObject]
...
extension SerialFilter
{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let movie = FilterSerials[indexPath.row]
        ...
        if indexPath.row + 1 == FilterSerials.count {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        ...
        controller.serial = FilterSerials[indexPath.row]
